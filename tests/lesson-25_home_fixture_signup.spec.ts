...
import { test as fixtureTest } from '../utils/fixtures/pagesFixtures';
...
// ℹ️ In addition to "lesson-25_fixtures_storage_garage.spec.ts" 
// in this "lesson-25_home_fixture_signup.spec.ts" the following changes 
// have been made.
// The happy-path Sign Up test was moved into a separate 
// "fixtureTest" describe that uses the "app" fixture, 
// while the validation tests remain non-fixture ("baseTest") 
// because they start from a unauthenticated page 
// and don’t need shared "storageState". 
// The import and test-data inconsistencies also have been fixed.


fixtureTest.describe('POM SignUp happy path with app fixture', () => {
  fixtureTest.beforeEach(async ({ app }) => {
    await app.homePage.navigate();
    await app.homePage.openSignUpForm();
  });

    fixtureTest('TEST-00 - Happy path - Sign up new user', async ({ app }) => {
        const signUpForm = new SignUpForm(app.page);
...
baseTest.describe('POM SignUp validation without fixture', () => {
    ...
    baseTest('TEST-01(A) POS - Reg form, Name - Minimum length (min)', async () => {
    ...
    baseTest('TEST-01(B) POS - Reg form, Name - Maximum length (max)', async () => {
    ...
    baseTest('TEST-01(C) NEG - Reg form, Name - Empty field', async () => {
        await signUpForm.triggerErrorOnField('firstName');
        await expect(signUpForm.emptyNameMessage).toBeVisible();
        await signUpForm.expectNameHasRedBorder();
    });

    baseTest('TEST-01(D) NEG - Reg form, Name - Wrong data', async () => {
    ...
    baseTest('TEST-01(E) NEG - Reg form, Name - Wrong length (less than min)', async () => {
    ...
    baseTest('TEST-01(F) NEG - Reg form, Name - Wrong length (greater than max)', async () => {
    ...
    baseTest('TEST-02(A) POS - Reg form, Last Name - Minimum length (min)', async () => {
    ...
    baseTest('TEST-02(B) POS - Reg form, Last Name - Maximum length (max)', async ({ page }) => {
    ...
    baseTest('TEST-02(C) NEG - Reg form, Last Name - Empty', async () => {
        await signUpForm.triggerErrorOnField('lastName');
        await expect(signUpForm.emptyLastNameMessage).toBeVisible();
        await signUpForm.expectLastNameHasRedBorder();
    });

    baseTest('TEST-02(D) NEG - Reg form, Last Name - Wrong data', async () => {
    ...
    baseTest('TEST-02(E) NEG - Reg form, Last Name - Wrong length (less than min)', async () => {
    ...
    baseTest('TEST-02(F) NEG - Reg form, Last Name - Wrong length (greater than max)', async () => {
    ...
    baseTest('TEST-03(A) POS - Reg form, Email - Valid data', async () => {
    ...
    baseTest('TEST-03(B) NEG - Reg form, Email - Empty', async () => {
        await signUpForm.triggerErrorOnField('email');
        await expect(signUpForm.emptyEmailMessage).toBeVisible();
        await signUpForm.expectEmailHasRedBorder();
    });

    baseTest('TEST-03(C) NEG - Reg form, Email - Wrong data', async () => {
    ...
    baseTest('TEST-04(A) POS - Reg form, Password - Valid data, minimum length', async () => {
    ...
    baseTest('TEST-04(B) POS - Reg form, Password - Valid data, maximum length', async () => {
    ...
    baseTest('TEST-04(C) NEG - Reg form, Password - Empty', async () => {
        await signUpForm.triggerErrorOnField('password');
        await expect(signUpForm.emptyPasswordMessage).toBeVisible();
        await signUpForm.expectPasswordHasRedBorder();
    });

    baseTest('TEST-04(D) NEG - Reg form, Password - Wrong data, alphas missed', async () => {
    ...
    baseTest('TEST-04(E) NEG - Reg form, Password - Wrong data, numbers missed', async ({ page }) => {
    ...
    baseTest('TEST-04(F) NEG - Reg form, Password - Wrong data, capital missed', async ({ page }) => {
    ...
    baseTest('TEST-04(G) NEG - Reg form, Password - Wrong data, small missed', async ({ page }) => {
    ...
    baseTest('TEST-04(H) NEG - Reg form, Password - Wrong data, length less than min', async () => {
    ...
    baseTest('TEST-04(I) NEG - Reg form, Password - Wrong data, length greater than max', async ({ page }) => {
    ...
    baseTest('TEST-05(A) POS - Reg form, Repeat password - Fully match', async () => {
    ...
    baseTest('TEST-05(B) NEG - Reg form, Repeat password - Empty', async () => {
    ...
    baseTest('TEST-05(C) NEG - Reg form, Repeat password - Do not match (partly, 1 char less)', async () => {
        const password = generateRandomPassword(10);
    ...
    baseTest('TEST-05(D) NEG - Reg form, Repeat password - Do not match (partly, 1 char greater)', async () => {
        const password = generateRandomPassword(10);
    ...
    baseTest('TEST-05(E) NEG - Reg form, Repeat password - Do not match (partly, 1 char invalid)', async () => {
        const password = generateRandomPassword(10);
    ...
    baseTest('TEST-06(A) POS - Reg form, Register button - Enabled when all fields contain valid data', async () => {
    ...
    baseTest('TEST-06(B) NEG - Reg form, Register button - Disabled when all fields empty', async () => {
        await expect(signUpForm.registerButton).toBeDisabled();
    });

    baseTest('TEST-06(C) NEG - Reg form, Register button - Disabled when at least one field contains invalid data', async () => {
        const password = generateRandomPassword(10);
        await signUpForm.fillInWithCredentials(
            'x',
            'TestLastName',
