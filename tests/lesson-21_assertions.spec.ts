...
test.describe('Auto retrying assertions', () => {
    ...
    test('toContainText', async ({page}) => {
        await page.locator('.header_signin').click();
        await expect(page.locator('.modal-title')).toContainText('og');         
    })

    test('toHaveValue', async ({page}) => {
        await page.locator('.header_signin').click();
        await page.locator('#signinEmail').fill('sample@gmail.com');
        await expect(page.locator('#signinEmail')).toHaveValue('sample@gmail.com');
    })
...
test.describe('Non-retrying assertions', () => {
    ...
    test('toBeGreater', () => {
        const result2 = 10 + 12;
        expect(result2).toBeGreaterThan(20);
    })
